...
export const NowPlaying = () => {
  ...
  const { data: lastPlayedData, error: lastPlayedError } = useSWR<{
    data: SongData;
  }>(
  ...
  const isLoading =
    !currentData && !lastPlayedData && !currentError && !lastPlayedError;

  // Use current playing song if available, otherwise try to use last played
  // Safely handle when last played data might not be available due to missing scope
  const songData = currentData?.data?.isPlaying
    ? currentData?.data
    : lastPlayedData?.data || null;
