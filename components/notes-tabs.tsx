...
export function NotesTabs({ notes, links }: { notes: React.ReactNode; links: React.ReactNode }) {
...
function TabButton({
  ...
  return (
    ...
    >
      {children}
      {active && <span className="absolute bottom-0 left-3 right-3 h-px bg-foreground" />}
    </button>
  );
}
