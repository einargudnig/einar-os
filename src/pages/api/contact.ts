...
// Best-effort throttle. Worker isolates don't share memory and are recycled
// freely, so this stops casual hammering rather than a determined flood. Only
// delivery attempts count against it — a typo'd email shouldn't lock anyone out.
const sends = new Map<string, number[]>();

const recentSends = (ip: string) => {
...
export const POST: APIRoute = async ({ request, clientAddress }) => {
  // clientAddress is CF-Connecting-IP under the Cloudflare adapter; the header
  // fallback covers `astro dev`, where there is no edge in front of the server.
  const ip =
